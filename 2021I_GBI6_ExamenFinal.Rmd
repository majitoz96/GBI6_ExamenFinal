...
library(sjPlot)
library(sjmisc)
CAdeMama <- TCA %>% group_by(gen) %>% mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
  select(-row)
descr(CAdeMama) %>% tab_df(title = "Estadistica descriptiva del cancer de MAMA en los diferentes nivel de expresion en los genes",
                             file = "C:/Users/Maria/Desktop/sexto semeste/bioinformatica/git'/GBI6_ExamenFinal/Result/mRNA_expr_summary.doc") 
...
cancertype_boxplots
ggsave("C:/Users/Maria/Desktop/sexto semeste/bioinformatica/git'/GBI6_ExamenFinal/Result/boxplot3.png", width = 6, height = 8, dpi = 300)
...
ggsave("C:/Users/Maria/Desktop/sexto semeste/bioinformatica/git'/GBI6_ExamenFinal/Result/regresion11.pdf", width = 5, height = 9, dpi = 300)
...
```{tex}
# En la figura boxplot se aprecia que la mediana no esta en el centro de lo caja por lo que puede presentar una asimetria negativa; por otra parte en la regresion se aprecian una distribucion normal debido a que existen muchos valores que estan fuera del rango permitido
